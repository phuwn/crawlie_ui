...
const { q } = storeToRefs(keywordStore())
const { fetchKeywords } = keywordStore()

</script>

<template>
	<form class="w-96" @submit.prevent="fetchKeywords">
		...
		<div class="relative w-full">
			...
			<input type="search" id="default-search"
				...
				placeholder="Search Keywords..." required v-model="q" />
			<button type="button"
				...
				@click="fetchKeywords">Search</button>
		</div>
	</form>
</template>